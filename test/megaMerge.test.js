...
describe("megaMerge", () => {
  it("should find all conflict SKU in two barcode list", async () => {
    ...
    expect(findConflictedSKUByBarcode(barcodeA, barcodeB)).toEqual(
      expectedConflictBarcode
    );
  });
  ...
  it("should remove the row that contains the SKU that is conflict", async () => {
    ...
    expect(catalog).not.toEqual(
      removeConflictProductBySKU(catalog, conflictBarcode)
    );
    expect(removeConflictProductBySKU(catalog, conflictBarcode)).toEqual(
      CatalogBWithoutConflict
    );
  });
  ...
  it("should merge two catalog without any conflict", async () => {
    ...
    expect(await megaMerge(catalogA, catalogB, barcodesA, barcodesB)).toEqual(
      mergedSuperset
    );
  });
});
